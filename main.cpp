...
std::vector<float> linspace(float start, float end, int count){
    ...
    if (count == 1){
        ...
        float delta = (end - start) / float(count - 1);
        for(int i=1; i < count-1; i++){
            linspace_vec.push_back(start + delta*i);
        }
...
int main(int argc, char **argv) {
    ...
    while ( 1 ) {
        static struct option long_options[] =
                {
                        ...
                        {"mem_start",     optional_argument,       0, 's'},
                        {"mem_end",  optional_argument,       0, 'e'},
                        {"mem_count",  optional_argument, 0, 'm'},
                        {"run_count",  optional_argument, 0, 'r'},
                        {"Fiducial",    no_argument, 0, 't'},
                        {"Plot",    no_argument, 0, 'p'},
                        {"FFTW",    no_argument, 0, 'f'},
        ...
        option = getopt_long (argc, argv, "s:e:m:r:tpfc",long_options, &option_index);
        ...
        switch (option) {
            case 's':
                ...
                break;
            case 'e':
                mem_end = float(strtod(optarg, nullptr));
                if (mem_end < mem_start)
                    throw std::invalid_argument("mem_end (-e) must be greater than or equal to mem_start (-s)");
                break;
            case 'm':
    ...
    /// \n
    /// \n
    /// START ############# Timed Run Section #############
    //

    std::vector<float> memory_sizes = double_space(mem_start, mem_count);//linspace(mem_start, mem_end, mem_count);
    for ( float i : memory_sizes ){
        std::cout << i << ", ";
    }
    std::cout << "\n";
...
//    std::vector<float> memory_sizes = {2000};
    ...
    //
    /// ############# Timed Run Section ############# END
    /// \n
    /// \n
    /// START ############# Individual Run Example Section #############
    /// \n
    /// START FFTW
    //
